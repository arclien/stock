...
def calc_stock_volume(raw_csv_file, calc_csv_file, stock_code, stock_name, nation, alert_percent, alert_price):
    if os.path.exists(raw_csv_file):
        ...
        for day in VOLUME_CALC_LENGTH:
            df = df.tail(day)
            alarm_message += calculate(day, df, calculated_row,
                                       df_today_volume, df_today_price, alert_percent)

        # for alert_price in alert_prices:
        alarm_message += check_alert_price(df_prev_day, df_today, alert_price)
...
def check_alert_price(df_yesterday, df_today, alert_price):
    if df_today.iloc[0]['Volume'] == 0 or df_yesterday.iloc[0]['Volume'] == 0:
        return ""
